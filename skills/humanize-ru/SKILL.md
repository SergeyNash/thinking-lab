...
## Thinking Lab routing

This copy lives in Thinking Lab (`skills/humanize-ru/`).

**Use this skill when:**
- removing AI markers, checking detector-style concerns, or fixing text that "sounds machine-generated";
- a final publication pass for a Russian article or draft focused on register-safe de-AI.

**Do not use this skill when:**
- running research, modules, or output choice → `skills/thinking-lab-content`;
- voice/mode adaptation without marker diagnosis → `skills/russian-humanizer`.

**Defaults in Thinking Lab:**
- For articles and essays, if register is unclear, use **Публицистический** (same as upstream default when unsure).
- Do not invent facts or specifics.
- Do not polish weak research into confident prose — if reasoning is thin, stop and point to `communication/HUMANIZER_RULES.md` / research modules.
- After this pass in the full pipeline, continue with `humanizer/HUMANIZER_CORE.md` and `communication/HUMANIZER_RULES.md`.
