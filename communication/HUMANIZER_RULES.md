...
[`humanizer-ru`](https://github.com/SergeNS-mne/humanizer-ru) for Thinking Lab.

It extends the existing Thinking Lab style system. It does not replace
`STYLE.md`, `RUSSIAN_ANTI_PATTERNS.md`, research method, evidence rules, or the
humanized product/JTBD voice.

The humanizer layer is used only during drafting, editing, and humanization of
Russian public-facing text.

---

## 1. Purpose

`HUMANIZER_RULES.md` helps remove:
...
4. apply this adapted humanizer layer;
5. adapt to the selected output format.

If a humanizer rule would weaken the thought, ignore the rule.

If a text is weak by reasoning, return it to reasoning. Do not humanize empty
thinking into confident prose.

---

## 3. Output-Type Presets

Use the same rules with different strictness.

### Telegram Note

Allow more spoken rhythm, first-person thinking, and mild roughness. Do not add
intentional mistakes. Keep the idea concrete.

### LinkedIn Post

Keep the text professional and readable. Remove LinkedIn theater, self-branding,
engagement bait, fake lessons, and over-polished symmetry.

### Essay

Prefer cleaner structure, calmer transitions, and stronger continuity. Keep the
voice alive, but reduce chatty shortcuts.

### Working Note

Allow unfinished thought, open questions, and visible uncertainty. Do not force
a conclusion.

### Unfinished Research

Preserve live exploration. Avoid pretending that the answer is ready.

### Failure Analysis

Use precise, non-moralizing language. Do not turn failure into a motivational
lesson.
...
Exceptions: not used by default in Thinking Lab public outputs.
...
Exceptions: one fragment may work as a spoken beat in Telegram.
...
Exceptions: spoken Telegram style may keep occasional "довольно" or "очень".
...
## 13. Future Voice Passport

Thinking Lab may later support a local author voice passport, but this version
does not create `.humanizer/`, `voice.json`, sessions, or auto-logs.
...
The voice passport must never override research quality or evidence rules.

---

## 14. Humanizer Pass

Before returning a public-facing Russian draft:

1. Run `RUSSIAN_ANTI_PATTERNS.md`.
2. Run this humanizer reference.
3. Check product/JTBD voice again.
4. Check that meaning, uncertainty, evidence, and limits survived.
5. Adapt only then to the output format.
